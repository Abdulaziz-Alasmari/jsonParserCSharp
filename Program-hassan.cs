...
namespace First
{
    ...
    public class JsonTokenizer : Tokenizable
    {
        public override bool tokenizable(Tokenizer t)
        {
            return (t.input.peek() == '{' || t.input.peek() == '[');
        }
        ...
        public override Token tokenize(Tokenizer t)
        {
            ...
            {
                if (t.input.Character == '"')
                {
                    while (t.input.peek() != '"' && t.input.peek() != '}')
                    {
                        n.key += t.input.step().Character;
                    }
                    t.input.step(3);
                    ...
                    if (t.input.Character =='"')
                    {
                        while (t.input.peek() != '"')
                        {
                            n.value += t.input.step().Character;
                        }
                        t.input.step(2);
                    }
                    else if (t.input.Character =='{' )
                    {
                        n.value = "object";
                    }
                    else if (t.input.Character=='[' )
                    ...
                    else if (Char.IsDigit(t.input.Character))
                    {
                        n.value += t.input.Character.ToString();
                        while (t.input.peek() != ',' && t.input.hasMore() && t.input.peek() != '}')
                        {
                            n.value += t.input.step().Character;
                        }
                    ...
                    else if (Char.ToLower(t.input.Character) == 't')
                    {
                        n.value = "True";
                        t.input.step(4);
                    }
                    else if (Char.ToLower(t.input.Character) == 'f')
                    {
                        n.value = "False";
                        t.input.step(4);
                    }
                    else if (Char.ToLower(t.input.Character) == 'n')
                    {
                        n.value = "Null";
                        t.input.step(4);
                    }
    ...
    class Program
    {
        ...
        {
            Tokenizer t = new Tokenizer(new Input("{\"glossary\":{\"title\":\"example glossary\",\"name\":\"hassan\",\"age\":25564,\"isit\":true,\"isnot\":false,\"glossary222\":{\"title22\":\" glossary22\",\"name22\":\"hassan22\"}\"arr\":[\"GML\",\"XML\"],\"isnot\":false}}}"), new Tokenizable[] {
                new JsonTokenizer(),
            }); ;
            Console.WriteLine("{\"glossary\":{\"title\":\"example glossary\",\"name\":\"hassan\",\"age\":25564,\"isit\":true,\"isnot\":false,\"glossary222\":{\"title22\":\" glossary22\",\"name22\":\"hassan22\"}\"arr\":[\"GML\",\"XML\"],\"isnot\":false}}}");
